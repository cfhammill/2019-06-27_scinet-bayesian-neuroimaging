...
- To use the singularity container you will need to set an environment variable with a temporary RStudio password.
...
singularity exec \
  ...
  /bb/scinet/course/ss2018/3_bm/7_mrir/RMINC-develop-2018-06-12-85c02d5c7b0c.img \
  rserver \
  --auth-none 0 \
...
Then navigate to localhost:8787 in your browser, use your scinet username and 
`$RSTUDIO_PASSWORD$` 
...
$$ \mathbf{\epsilon} \sim \mathfrak{N}(0, \sigma) $$
...
blm <- stan_glm(brain_volume ~ age, data = ex,
                prior = normal(0, 5))
```
...
mcmc_areas(as.matrix(blm), pars = "age")
```
...
```{r}
blm_post <- as.matrix(blm)

str(blm_post[,"age"])
summary(blm_post[,"age"])
```
